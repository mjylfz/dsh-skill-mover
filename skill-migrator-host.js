...
function parseFrontmatter(text) {
  ...
  while (i < lines.length) {
    ...
    if (/^[|>][-+]?$/.test(value)) {
      const parts = []
      i += 1
      while (i < lines.length && /^[ \t]/.test(lines[i])) { parts.push(lines[i].replace(/^[ \t]+/, '')); i += 1 }
...
module.exports = function makeHostPlugin() {
  return {
    apply(ctx) {
      let envCache = null
      ...
      async function detectEnv() {
        ...
        if (!shell) { envCache = { platform: 'unknown', home: null }; return envCache }
        try {
      ...
      async function pathExists(fs, path) {
        try { const t = await fs.resolve(path); const st = await fs.stat(t); return !!st } catch (e) { return false }
      }

      async function readJson(fs, path) {
        try { const t = await fs.resolve(path); return JSON.parse(await fs.readText(t)) } catch (e) { return null }
      }
      ...
      async function readSkill(fs, skillPath, dirPath, env) {
        ...
        try {
          ...
          if (!fm) {
            ...
            if (!fm.description) out.warnings.push('frontmatter 缺少 description(DSH 无法自动发现)')
            out.description = firstLine(fm.description, 140)
            if (fm.version) out.version = String(fm.version)
          }
        } catch (e) {
          out.warnings.push('读取 SKILL.md 失败: ' + errMsg(e))
        }
        for (const dep of DEP_FILES) {
          if (await pathExists(fs, dirPath + sep + dep)) out.deps.push(dep)
        }
        return out
      }
      ...
      async function walkDir(fs, dirPath, depth, source, env, out) {
        ...
        for (const entry of entries) {
          ...
          if (SKIP_NAMES.includes(name) || (source.skipNames || []).includes(name)) continue
      ...
      async function scanSource(source, env, overrides) {
        ...
        if (!env.home || env.platform === 'unknown') return out
        ...
        for (const rel of rels) {
          ...
          let target
          try { target = await fs.resolve(rootPath) } catch (e) { continue }
          let entries
      ...
      function buildGroups(sources, installed, env) {
        ...
        for (const c of candidates) {
          if (seen.has(c.path)) { seen.get(c.path).duplicates.push(c.sourceId); continue }
        ...
        for (const entry of map) {
          ...
          for (const c of cands) {
            ...
            for (const d of c.deps) if (!deps.includes(d)) deps.push(d)
          }
      ...
      async function fixFrontmatterName(fs, skillPath, slug) {
        ...
        let fmEnd = -1
        for (let i = 1; i < lines.length; i += 1) { if (lines[i].trim() === '---') { fmEnd = i; break } }
        if (fmEnd < 0) return
        let replaced = false
        for (let i = 1; i < fmEnd; i += 1) {
          if (/^name\s*:/.test(lines[i])) { lines[i] = 'name: ' + slug; replaced = true; break }
        }
        if (!replaced) lines.splice(1, 0, 'name: ' + slug)
      ...
      async function runMigrate(payload) {
        ...
        const results = []
        if (!targetRoot) return { results: [{ slug: '', status: 'fail', error: '无法确定 DSH skills 根目录' }] }
        ...
        for (const sel of payload.selections || []) {
          ...
          if (!group || !cand) { results.push({ slug: sel.slug, status: 'fail', error: '未找到候选技能' }); continue }
      ...
      async function runScan(overrides) {
        ...
        const sources = []
        for (const source of SOURCES) sources.push(await scanSource(source, env, overrides || {}))
        const installed = new Map()
        if (targetRoot) {
          try {
            const t = await fs.resolve(targetRoot)
            const entries = await fs.listDir(t)
            for (const e of entries) if (e.type === 'directory') installed.set(e.name, e.target.displayPath)
      ...
      const disposers = [
        harness.handle('scan', async (args) => {
          try {
            const overrides = args && args.overrides ? args.overrides : {}
            return { ok: true, ...await runScan(overrides) }
          } catch (e) {
            return { ok: false, error: errMsg(e) }
          }
        }),
        harness.handle('migrate', async (args) => {
          try { return { ok: true, ...await runMigrate(args || {}) } }
          catch (e) { return { ok: false, error: errMsg(e) } }
        }),
        harness.handle('remove', async (args) => {
          try { return { ok: true, ...await runRemove(args || {}) } }
          catch (e) { return { ok: false, error: errMsg(e) } }
        }),
        harness.handle('detect', async () => {
          try { return { ok: true, env: await detectEnv() } }
          catch (e) { return { ok: false, error: errMsg(e) } }
        }),
      ]
