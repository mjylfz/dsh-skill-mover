// DSH Skill Migrator — Client half: settings page UI (plain JS, React.createElement only)

module.exports = function makeClientPlugin() {
  return {
    async apply(ctx) {
      ...
      slots.inject('settings.section', () => slots.register(
        { name: 'settings.section', id: 'skill-migrator', order: 50, label: () => 'Skill 迁移' },
        () => React.createElement(SkillMigratorPage, null),
      ))
    },
  }
}
...
function SkillMigratorPage() {
  const [phase, setPhase] = React.useState('loading')
  const [scan, setScan] = React.useState(null)
  const [selected, setSelected] = React.useState({})
  const [mode, setMode] = React.useState('copy')
  const [overwrite, setOverwrite] = React.useState(false)
  const [results, setResults] = React.useState(null)
  const [error, setError] = React.useState(null)
  const [openConflict, setOpenConflict] = React.useState({})
  ...
  async function doScan() {
    ...
    setResults(null)
    try {
      const res = await host.call('scan', {})
      if (!res || res.ok === false) { setError((res && res.error) || '扫描失败'); setPhase('ready'); return }
      setScan(res)
      const sel = {}
  ...
  async function doMigrate() {
    ...
    setError(null)
    try {
      const res = await host.call('migrate', { selections, mode: 'copy', overwrite })
      if (!res || res.ok === false) { setError((res && res.error) || '迁移失败'); setPhase('ready'); return }
  ...
  async function doRemove(slugs) {
    setPhase('migrating')
    try {
      const res = await host.call('remove', { slugs })
      if (!res || res.ok === false) { setError((res && res.error) || '移除失败'); setPhase('done'); return }
